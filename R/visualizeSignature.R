...
visPMS_ind <- function(vF, numBases, baseCol = NA, trDir, charSize = 1.2) {
  ...
  for(l in 1:numBases) {
    ...
    for(w in 1:4) {
      ...
      if (endx - startx > 1 / 4 & !is.null(charSize) & charSize > 0) {
  ...
  for (w in 1:4) {
    ...
    for(ww in 1:4) {
      ...
      if ((endy - starty > 1 / 4) & (endx - startx > 1 / 4) & !is.null(charSize) & charSize > 0) {
  ...
  endy <- starty + v3[1];
  polygon(c(startx, endx, endx, startx), c(starty, starty, endy, endy), col=baseCol[5], border=F);
  if (endy - starty > 1 / 8 & !is.null(charSize) & charSize > 0) {
